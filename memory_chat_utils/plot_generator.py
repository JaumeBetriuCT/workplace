...
class CodeGeneratingAssistant:
    def __init__(self, data_path: str, model_name: str):
        self.semantic_engine = SemanticQueryEngine(data_path, model_name)
        self.df_tickets = pd.read_csv(data_path)
        #self.df_tickets['tags'] = self.df_tickets['tags'].apply(ast.literal_eval)
            ...
            with st.spinner('Generating plot... Please wait.'):
                try:
                    ...
                    if attempt == 0:  # Solo genera el código en el primer intento
                        python_code = self.semantic_engine.generate_visualization_code(user_prompt)
                    ...
                    python_code = self.semantic_engine.generate_visualization_code(feedback_prompt)
                    python_code = python_code.replace("```python", "").replace("```", "").strip()
